...
class SmartHistoricalCollector:
    """Smart historical data collector with rate limiting and error handling"""
    
    def __init__(self, db_path: str = "smart_historical_data.db"):
        # Setup logging
        ...
        # Comprehensive stock universe (prioritized by importance)
        self.stock_universe = {
            # Tier 1: Major International Stocks (most important)
            'AAPL': 'Apple Inc.',
            ...
            # Tier 3: Additional International Stocks
        ...
        # Collection settings (conservative to avoid rate limiting)
        ...
        # Progress tracking
        self.progress_file = "smart_collection_progress.json"
        self.load_progress()
    ...
    def get_sector(self, ticker: str) -> str:
        ...
        sector_map = {
            ...
            'META': 'Technology', 'NFLX': 'Technology', 'CRM': 'Technology',
            'ADBE': 'Technology', 'ORCL': 'Technology', 'CSCO': 'Technology',
            'INTC': 'Technology', 'IBM': 'Technology',
            'BRK-B': 'Financial', 'JPM': 'Financial', 'BAC': 'Financial',
            'WFC': 'Financial', 'AFI.NZ': 'Financial', 'WBC.NZ': 'Financial',
            'ANZ.NZ': 'Financial',
            'JNJ': 'Healthcare', 'FPH.NZ': 'Healthcare',
            'PG': 'Consumer', 'KO': 'Consumer', 'PEP': 'Consumer',
            'WMT': 'Consumer', 'HD': 'Consumer', 'ATM.NZ': 'Consumer',
            'MCD': 'Consumer', 'NKE': 'Consumer', 'DIS': 'Consumer',
            'CVX': 'Energy', 'XOM': 'Energy',
            'MEL.NZ': 'Utilities', 'VCT.NZ': 'Utilities', 'MCY.NZ': 'Utilities',
            'GNE.NZ': 'Utilities', 'CEN.NZ': 'Utilities',
            'AIA.NZ': 'Industrial', 'IFT.NZ': 'Industrial', 'MFT.NZ': 'Industrial',
            'POT.NZ': 'Industrial', 'FBU.NZ': 'Industrial', 'AIR.NZ': 'Industrial',
            'BA': 'Industrial', 'CAT': 'Industrial', 'GE': 'Industrial',
            'F': 'Automotive', 'RTX': 'Aerospace',
            'SPK.NZ': 'Communication', 'CNU.NZ': 'Communication',
            'T': 'Communication', 'VZ': 'Communication',
            'SUM.NZ': 'Healthcare', 'RYM.NZ': 'Healthcare',
            'GMT.NZ': 'Real Estate', 'PCT.NZ': 'Real Estate', 'KPG.NZ': 'Real Estate',
            'MMM': 'Industrial', 'FCG.NZ': 'Consumer', 'EBO.NZ': 'Healthcare',
            'FRW.NZ': 'Industrial', 'GTK.NZ': 'Technology',
        }
    ...
    def run_collection_session(self, max_tickers: int = None):
        ...
        self.logger.info(f"Starting collection session for {len(pending_tickers)} tickers")
        ...
        for i, ticker in enumerate(pending_tickers):
            try:
                self.logger.info(f"Processing {ticker} ({i+1}/{len(pending_tickers)})")
    ...
    def log_collection_session(self, start_time: datetime, end_time: datetime,
        ...
        try:
            ...
            ''', (
                ...
                f"Smart collection session - Rate limited"
            ))
    ...
    def print_progress_summary(self):
        ...
        print("\n" + "="*80)
        print("SMART HISTORICAL DATA COLLECTION PROGRESS")
        print("="*80)
...
def main():
    """Main function for smart historical data collection"""
    collector = SmartHistoricalCollector()
    
    print("="*80)
    print("SMART HISTORICAL DATA COLLECTION")
    print("="*80)
    ...
    # Run collection session (process 10 tickers at a time to avoid rate limiting)
    records_collected, errors = collector.run_collection_session(max_tickers=10)
